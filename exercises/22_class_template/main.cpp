...
struct Tensor4D {
    ...
    Tensor4D &operator+=(Tensor4D const &others) {
        unsigned int s3 = 1;
        unsigned int s2 = others.shape[3];
        unsigned int s1 = others.shape[2] * others.shape[3];
        unsigned int s0 = others.shape[1] * others.shape[2] * others.shape[3];
        T* ptr = data;
            ...
            for (unsigned int j = 0; j < shape[1]; ++j) {
                ...
                for (unsigned int k = 0; k < shape[2]; ++k) {
                    ...
                    for (unsigned int l = 0; l < shape[3]; ++l) {
                        ...
                        unsigned int idx = oi * s0 + oj * s1 + ok * s2 + ol * s3;
                        *ptr += others.data[idx];
                        ptr++; 
                    }
                }
